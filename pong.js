...
class Pong
{
    ...
    collide(player, ball)
    {
        if (player.left < ball.right && player.right > ball.left &&
            ...
            //redirect and accelerate the ball
            ball.vel.x = -ball.vel.x * 1.07;
            const len = ball.vel.len;
            ball.vel.y += player.vel.y * .2;
            ball.vel.len = len;
            //ensure ball is in valid location and not stuck in a paddle
            ball.pos.x = player.pos.x + (ball.vel.x < 0 ? (-(player.size.x / 2) - (ball.size.x / 2)) - 1 : player.size.x / 2 + (ball.size.x / 2));
            //player scores for blocking the ball 
            ++player.score;
        }
    }
    draw()
    ...
    drawScore()
    {
        const align = this._canvas.width / 2;
        const cw = this.CHAR_PIXEL * 4;
        const chars = this.players[0].score.toString().split('');
        const offset = align * (1) - (cw * chars.length / 2) + this.CHAR_PIXEL / 2;
